...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        mini_batch.src = (mini_batch.src[0].to(device), mini_batch.src[1]) # mini_batch.src[1] : length
        ...
        # |x| = (batch_size, length) & length tensor
        # |y| = (batch_size, length)
        ...
        with autocast(not engine.config.off_autocast):
            ...
            loss = engine.crit(
                y_hat.contiguous().view(-1, y_hat.size(-1)), # (batch_size * length, output_size)
                y.contiguous().view(-1) # (batch_size * length,)
            )
